...
const fixture = Object.freeze({
  ...
  objectPrefix: "imports/11111111-1111-4111-8111-111111111111/22222222-2222-4222-8222-222222222222/",
});
...
test("cleanup is one transaction, exact-ID scoped, idempotent, and never SET ROLE", async () => {
  const calls = [];
  const client = { query: async (sql, values = []) => { calls.push([sql, values]); return { rowCount: 0, rows: [] }; }, release() {} };
  const target = { environment: "staging", projectId: "withered-bar-40598816", branchId: "br-wispy-flower-avd4hssa", endpointId: "ep-lingering-pine-avbdti90", database: "neondb", directHostPrefix: "ep-lingering-pine-avbdti90." };
  ...
  assert.doesNotMatch(sql, /delete\s+from\s+public\.\w+\s*;\s*$/im);
