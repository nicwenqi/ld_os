...
export function WorkbookSetupStep({propertyId,repository,realMode,complete,persistedInspected,onInspected,onConfirm,notify}:Props){
  const input=useRef<HTMLInputElement>(null);const[selectedFile,setSelectedFile]=useState<File|null>(null);const[fileState,setFileState]=useState<FileState>(persistedInspected?"staged":"none");const[fileName,setFileName]=useState(persistedInspected?"已保存的员工主数据批次":"");const[inspection,setInspection]=useState<InspectionView|null>(null);const[busy,setBusy]=useState(false);const[error,setError]=useState("");
  const choose=(file?:File)=>{if(!file)return;setSelectedFile(file);setFileName(file.name);setFileState("selected");setInspection(null);setError("")};
  const inspect=async()=>{if(!selectedFile&&!persistedInspected)return;setBusy(true);setError("");try{
    let result:InspectionView;
    if(realMode){
      if(!selectedFile)throw new Error("请选择要检查的工作簿");
      const form=new FormData();form.set("file",selectedFile);const response=await fetch("/api/import/inspect",{method:"POST",body:form});const payload=await response.json();if(!response.ok)throw new Error(payload.message??"工作簿检查失败");result=payload;
    }else{
