...
export function createSupabasePositionRepository(client: Client): PositionRepository {
  ...
  return {
    ...
    async previewSourceImpact(id) { const { data, error } = await client.from("position_aliases").select("id").eq("id", id).single(); if (error || !data) throw new Error("未找到职位来源标签"); return { syntheticEmployeeCount: 0, departmentNames: [] }; },
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mapSource(row: any): PositionSourceLabel { return { id: row.id, propertyId: row.property_id, sourceSystem: row.source_system, sourceSheet: row.source_sheet ?? "历史批准来源", sourceValue: row.source_value, normalizedSourceValue: row.normalized_source_value, syntheticEmployeeCount: Number(row.source_row_count??0), departmentNames: [], suggestedPositionId: row.target_position_id, suggestedFamilyId: row.target_position_family_id, confidence: row.approved_at ? 100 : 0, suggestionReason: row.approved_at ? "来自已确认的历史映射" : "尚未建立批准规则", targetPositionId: row.target_position_id, targetPositionFamilyId: row.target_position_family_id, externalRoleCode: row.external_role_code, externalRoleName: row.external_role_name, resolutionStatus: row.resolution_status }; }
