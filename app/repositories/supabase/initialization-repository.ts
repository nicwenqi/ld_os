...
export function createSupabaseInitializationRepository(client: Client): InitializationRepository { return {
  ...
  async saveNavigation(propertyId,lastActiveStep,expectedVersion) { const {data,error}=await client.rpc("save_property_initialization_navigation",{p_property_id:propertyId,p_last_active_step:lastActiveStep,p_expected_version:expectedVersion??null});if(error)throw new Error(`无法保存浏览位置：${error.message}`);return data; },
