import type { ApprovePositionMappingInput, PositionRepository, SavePositionFamilyInput, SavePositionInput } from "../contracts/position-repository.ts";
...
export function createMockPositionRepository(): PositionRepository {
  let families: PositionFamily[] = [["f-manager","manager","管理人员","Manager"],["f-supervisor","supervisor","主管人员","Supervisor"],["f-associate","associate","一线员工","Associate"],["f-culinary","culinary","厨房岗位","Culinary"],["f-engineering","engineering","工程岗位","Engineering"]].map((item, index) => ({ id: item[0], tenantId, propertyId, code: item[1], nameZh: item[2], nameEn: item[3], description: "Synthetic position family", sortOrder: (index + 1) * 10, isActive: true }));
  let positions: OfficialPosition[] = [
  ...
  let sources: PositionSourceLabel[] = [
  ...
  return {
    ...
    async savePositionFamily(input: SavePositionFamilyInput) { const current = input.id ? families.find(item => item.id === input.id) : undefined; if (current) Object.assign(current, input); else { const created = { ...input, id: crypto.randomUUID() } as PositionFamily; families.push(created); return copy(created); } return copy(current); },
    ...
    async previewSourceImpact(id) { const current = sources.find(item => item.id === id); if (!current) throw new Error("未找到职位来源标签"); return { syntheticEmployeeCount: current.syntheticEmployeeCount, departmentNames: [...current.departmentNames] }; },
