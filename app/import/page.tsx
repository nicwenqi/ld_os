...
function Page() {
  ...
  useEscapeDismiss(confirmOpen, () => setConfirmOpen(false));
  ...
  return <AppShell><div className="page-wrap import-page">
    ...
    <nav className="import-steps" aria-label="导入步骤">{steps.map(([number, zh, en]) => <button key={number} className={`${step === number ? "active" : ""} ${step > number ? "done" : ""}`} disabled={number > step} title={number > step ? "请先完成当前步骤" : `返回${zh}`} onClick={() => setStep(number)}><span>{step > number ? "✓" : number}</span><div><strong>{zh}</strong><small>{en}</small></div></button>)}</nav>
    ...
    {confirmOpen && <div className="dialog-backdrop" onMouseDown={() => setConfirmOpen(false)}><div className="dialog import-confirm-dialog" role="dialog" aria-modal="true" aria-label="确认模拟导入" onMouseDown={event => event.stopPropagation()}><span>FINAL CHECK</span><h2>确认执行模拟导入？</h2><p>系统将展示完整完成状态，但不会影响酒店现有资料。</p><div><strong>18 新增</strong><strong>7 更新</strong><strong>{6 - resolvedIssues.length} 排除 / 待处理</strong></div><footer><button onClick={() => setConfirmOpen(false)}>再检查一次</button><button onClick={() => { setConfirmOpen(false); setImportComplete(true); showToast("模拟导入已成功完成"); }}>确认并开始导入</button></footer></div></div>}
  </div></AppShell>;
}
