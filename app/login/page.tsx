...
import type { PropertyContext } from "../repositories/contracts/models.ts";
import "./login.css";

function LoginExperience(){const router=useRouter();const{login,status,session}=useAuthSession();const registry=useMemo(()=>createRepositoryRegistry(),[]);const[property,setProperty]=useState<PropertyContext|null>(null);const[loginId,setLoginId]=useState("");const[password,setPassword]=useState("");const[loading,setLoading]=useState(false);const[error,setError]=useState("");
  useEffect(()=>{const hostname=registry.environment.dataMode==="mock"?"training-demo.example.test":registry.environment.devPropertyHostname??registry.environment.previewPropertyHostname??window.location.hostname;void registry.property.resolveContext(hostname).then(setProperty)},[registry]);
