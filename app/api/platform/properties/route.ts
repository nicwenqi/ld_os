...
export async function POST(request: Request) {
  try {
    ...
    const admin = createServerAdminClient(); const internalEmail = `${crypto.randomUUID()}@accounts.ldchub.cn`; const temporaryPassword = generateTemporaryPassword();
    const { data: auth, error: authError } = await admin.auth.admin.createUser({ email: internalEmail, password: temporaryPassword, email_confirm: true, app_metadata: { hotel_ld_internal_account: true } });
    ...
    try {
      ...
      return response({ source: "real", handoff: data, issuedTemporaryPassword: temporaryPassword }, actor.refreshedCookies);
