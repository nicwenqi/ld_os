...
export async function POST(request:Request){if(!isMock())return Response.json({message:"not found"},{status:404});const body=await request.json() as SaveWizardStepInput&{action?:"complete"|"navigate"};if(body.propertyId!==propertyId)return Response.json({message:"无权修改其他酒店初始化资料"},{status:403});if(body.expectedVersion&&body.expectedVersion!==state.version)return Response.json({message:"初始化进度已更新，请刷新后重试"},{status:409});if(body.action==="complete")state={...state,completedAt:new Date().toISOString(),version:state.version+1};else if(body.action==="navigate")state={...state,lastActiveStep:body.lastActiveStep,version:state.version+1};else state={...state,lastActiveStep:body.lastActiveStep,steps:{...state.steps,[body.stepKey]:{...state.steps[body.stepKey],explicitlyConfirmed:body.explicitlyConfirmed??state.steps[body.stepKey]?.explicitlyConfirmed,warning:body.warning??state.steps[body.stepKey]?.warning??null,blockingReason:body.blockingReason??state.steps[body.stepKey]?.blockingReason??null}},version:state.version+1};return noStore(state)}
