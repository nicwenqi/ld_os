...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    if ([...new URL(request.url).searchParams.keys()].length) throw new ImportMappingInputError("来源标签查询不接受参数");
    ...
    const result = await runAuthorizedNeonImportStaging(request, requestId, async scope => {
      ...
      return { ...value, sourceLabels: value.sourceLabels };
    });
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await runAuthorizedNeonImportStaging(request, requestId, scope => scope.mappingRepository.saveSourceLabelDecisions({ batchId, ...body }));
    return Response.json(result.data, { headers: result.headers });
  } catch (error) {
    return importStagingErrorResponse(inputError(error), requestId);
  }
