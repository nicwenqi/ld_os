  ...
  try {
    ...
    const result = await runAuthorizedNeonPositionWrite(
      ...
      (repository, scope) => repository.savePositionFamily(
        parseSavePositionFamilyInput(body, scope, canonicalId(id)),
      ),
    );
...
function canonicalId(value: string) {
  if (!UUID.test(value)) throw new OrganizationApiError(400, "职位族标识无效");
  return value.toLowerCase();
}

function rejectQueryParameters(request: Request) {
  if ([...new URL(request.url).searchParams.keys()].length > 0) {
    throw new OrganizationApiError(400, "职位族保存请求不接受查询参数");
  }
}

async function requestBody(request: Request): Promise<unknown> {
  try { return await request.json(); }
  catch { throw new OrganizationApiError(400, "职位族保存请求格式无效"); }
}
