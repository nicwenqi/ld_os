...
export async function POST(
  ...
  try {
    ...
    const input = parseMoveDepartmentInput(id, await requestBody(request));
    const result = await runAuthorizedNeonOrganizationWrite(
      request,
      requestId,
      repository => repository.moveNode(
        input.id,
        input.newParentId,
...
function rejectQueryParameters(request: Request) {
  if ([...new URL(request.url).searchParams.keys()].length > 0) {
    throw new OrganizationApiError(400, "部门移动请求不接受查询参数");
  }
}
...
async function requestBody(request: Request) {
  try {
    ...
    throw new OrganizationApiError(400, "部门移动请求格式无效");
  }
}
