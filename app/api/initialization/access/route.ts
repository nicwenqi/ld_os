...
export async function GET(request:Request){
  try{
    ...
    const [{data:assignments,error:assignmentError},{data:memberships,error:membershipError},{data:accounts,error:accountError}]=await Promise.all([
      admin.from("role_assignments").select("id,user_id,status,roles!inner(code,is_active)").eq("property_id",actor.propertyId).eq("status","active").eq("roles.is_active",true).in("roles.code",["property_ld_manager","department_training_admin"]),
    ...
    const departmentAssignments=activeAssignments.filter(item=>roleCode(item)==="department_training_admin");
    const {data:scopes,error:scopeError}=departmentAssignments.length
      ? await admin.from("trainer_scopes").select("role_assignment_id").eq("property_id",actor.propertyId).eq("is_active",true).in("role_assignment_id",departmentAssignments.map(item=>item.id))
      : {data:[],error:null};
    if(scopeError)throw new Error("部门范围状态读取失败");
    const scopedAssignmentIds=new Set((scopes??[]).map(item=>item.role_assignment_id));
    const activeDepartmentAdministratorIds=new Set(departmentAssignments.map(item=>item.user_id));
    const activeDepartmentAdministratorsWithScope=new Set(
      departmentAssignments
        .filter(item=>scopedAssignmentIds.has(item.id))
    ...
    return Response.json({
      ...
      activeDepartmentAdministrators:activeDepartmentAdministratorIds.size,
      activeDepartmentAdministratorsWithScope,
      departmentScopesResolved:activeDepartmentAdministratorsWithScope===activeDepartmentAdministratorIds.size,
