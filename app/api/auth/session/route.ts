...
export async function GET(request:Request){
  ...
  try{const identity=await resolveRequestAuthIdentity(request);if(!identity)return noStore(anonymousSession);const headers=new Headers({"Cache-Control":"no-store"});if(identity.refreshed)for(const value of authCookies(identity.accessToken,identity.refreshToken,environment.appEnv!=="local"))headers.append("Set-Cookie",value);return Response.json(await resolveSessionForAuthUser(identity.userId,identity.hostname),{headers})}catch{return noStore(anonymousSession)}
}
