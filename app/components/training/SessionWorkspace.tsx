      ...
      {editorOpen && (
        <div className="d2-editor-layer">
          <form className="d2-editor wide" onSubmit={event => { event.preventDefault(); void saveDraft(); }}>
            ...
            <fieldset><legend>当前场次准备边界</legend><label className="d2-check"><input type="checkbox" checked={form.materialsReady} onChange={e => updateForm("materialsReady", e.target.checked)} />负责人确认教材已准备</label><label className="d2-check"><input type="checkbox" checked={form.roomReady} onChange={e => updateForm("roomReady", e.target.checked)} />负责人确认场地布置已准备</label><Field label="出勤登记方式"><select value={form.preparationMode} onChange={e => updateForm("preparationMode", e.target.value as SessionForm["preparationMode"])}><option value="manual_only">仅人工见证</option><option value="qr_or_manual">二维码 Observation + 人工核对</option></select></Field></fieldset>
