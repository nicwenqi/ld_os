...
type FormState = { displayName: string; loginId: string; temporaryPassword: string };
const emptyForm: FormState = { displayName: "", loginId: "", temporaryPassword: "" };
...
export default function PropertyAccountsPage() {
  ...
  async function saveManager(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json() as { account?: PlatformManagerAccount; replacement?: { newManager?: PlatformManagerAccount }; message?: string };
      if (!response.ok) throw new Error(payload.message ?? "账号操作未完成");
      setForm(emptyForm); setReplaceFor(null); setNotice(replaceFor ? "新经理已建立，原经理已停用。请通过受控交接发放新登录信息。" : "经理账号已建立，首次登录需要修改密码。");
  ...
  async function resetPassword(account: PlatformManagerAccount) {
    const temporaryPassword = window.prompt("输入新的临时密码（至少 12 位，包含字母和数字）", "");
    if (!temporaryPassword) return;
    setBusy(true); setError(null); setNotice(null);
    try {
      const response = await fetch(`/api/platform/properties/${propertyId}/accounts`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accountId: account.accountId, expectedVersion: account.version, temporaryPassword }) });
      const payload = await response.json() as { message?: string };
      if (!response.ok) throw new Error(payload.message ?? "密码重置失败");
      setNotice("临时密码已更新；经理下次登录需要修改密码。"); setAccounts(await loadAccounts(propertyId));
