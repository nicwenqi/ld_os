...
const initialDraft: PropertyProvisioningDraft = {
  tenantId: "", propertyCode: "", preliminaryNameZh: "", preliminaryNameEn: "", brand: "", city: "", countryRegion: "CN", timezone: "Asia/Shanghai", defaultLanguage: "zh-CN", hostname: "", managerLoginId: "", managerDisplayName: "",
};
...
export default function NewPropertyProvisioningPage() {
  ...
  async function submit(operation: "preview" | "commit") {
    ...
    try {
      ...
      const payload = await response.json() as { preview?: Preview; handoff?: Handoff; issuedTemporaryPassword?: string; notice?: string; message?: string };
      ...
      if (operation === "commit" && payload.handoff) { setHandoff(payload.handoff); setIssuedTemporaryPassword(payload.issuedTemporaryPassword ?? null); setNotice(payload.notice ?? null); }
    } catch (reason) {
  ...
  return (
    <main className="platform-shell">
      <section className="platform-frame">
        <div className="platform-topline"><div className="platform-brand"><span className="platform-mark">L&D</span><span><strong>{session.displayName}</strong><small>平台开通控制面 · 不进入酒店业务空间</small></span></div><button className="platform-link" type="button" onClick={logout}>退出平台开通</button></div>
        <article className="platform-card">
          <header><p className="platform-kicker">C1 · PROPERTY PROVISIONING</p><h1>创建 Pilot Property</h1><p>此步骤只建立酒店容器、初始技术上下文与首位学习与发展经理交接。组织、员工与所有培训业务事实均不在此创建。</p></header>
          <form className="platform-form" onSubmit={previewProvisioning}>
            <section className="platform-section"><h2>Property 容器</h2><p>填写经批准的 Pilot 酒店基础身份。确认前仅生成可复核预览。</p><div className="platform-form-grid"><TextField label="Tenant 标识" name="tenantId" value={draft.tenantId} onChange={setField} /><TextField label="Property code" name="propertyCode" value={draft.propertyCode} onChange={setField} /><TextField label="酒店中文名称" name="preliminaryNameZh" value={draft.preliminaryNameZh} onChange={setField} /><TextField label="酒店英文名称" name="preliminaryNameEn" value={draft.preliminaryNameEn} onChange={setField} /><TextField label="品牌" name="brand" value={draft.brand} onChange={setField} /><TextField label="城市" name="city" value={draft.city} onChange={setField} /><TextField label="国家或地区" name="countryRegion" value={draft.countryRegion} onChange={setField} /><TextField label="酒店域名" name="hostname" value={draft.hostname} onChange={setField} /></div><div className="platform-form-grid"><SelectField label="时区" name="timezone" value={draft.timezone} onChange={setField} values={["Asia/Shanghai"]} /><SelectField label="默认语言" name="defaultLanguage" value={draft.defaultLanguage} onChange={setField} values={["zh-CN", "en-US"]} /></div></section>
            <section className="platform-section"><h2>首位学习与发展经理交接</h2><p>系统会在确认创建后生成 12 位随机临时密码，并只显示一次；经理必须完成首次密码更新，才能进入酒店管理空间。</p><div className="platform-form-grid"><TextField label="经理用户 ID" name="managerLoginId" value={draft.managerLoginId} onChange={setField} /><TextField label="经理姓名" name="managerDisplayName" value={draft.managerDisplayName} onChange={setField} /></div></section>
            {error ? <p className="platform-error" role="alert">{error}</p> : null}<div className="platform-actions"><button className="platform-primary" type="submit" disabled={saving || Boolean(handoff)}>{saving ? "处理中…" : "预览开通内容"}</button></div>
          </form>
          {preview ? <section className="platform-preview" aria-live="polite"><h2>开通预览</h2><p>确认以下内容后才会执行受控开通；预览将在 {new Date(preview.expiresAt).toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })} 失效。</p><PreviewDetails value={preview.normalized} /><div className="platform-actions"><button className="platform-secondary" type="button" onClick={() => setPreview(null)} disabled={saving}>返回修改</button><button className="platform-primary" type="button" onClick={() => submit("commit")} disabled={saving}>{saving ? "创建中…" : "确认创建 Property 并发出经理交接"}</button></div></section> : null}
          {handoff ? <section className="platform-success" aria-live="assertive"><strong>经理交接已建立</strong><p>酒店域名：{handoff.hostname}；首位经理：{handoff.managerDisplayName}。经理需完成首次密码更新后，才能进入其酒店管理空间。</p>{issuedTemporaryPassword ? <output>{issuedTemporaryPassword}</output> : null}{notice ? <p className="platform-notice">{notice}</p> : null}</section> : null}
        </article>
      </section>
    </main>
  );
