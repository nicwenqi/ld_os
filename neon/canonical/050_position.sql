...
create function app_private.neon_position_actor_can_read_position(p_id uuid) returns boolean language sql stable security invoker set search_path='' as $f$
  select app_private.neon_organization_actor_has_role('property_ld_manager')
    or not exists (
      select 1 from public.position_department_assignments a
      where a.position_id=p_id and a.property_id=app_private.current_actor_property_id() and a.is_active
    )
    or exists (
      select 1 from public.position_department_assignments a
      where a.position_id=p_id and a.property_id=app_private.current_actor_property_id() and a.is_active
        and app_private.neon_organization_actor_has_department_scope(a.department_id)
    )
$f$;
...
declare r jsonb; c integer; begin perform app_private.assert_neon_organization_hostname(p_hostname); perform app_private.assert_neon_organization_reader(); select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('id',family.id,'tenant_id',family.tenant_id,'property_id',family.property_id,'code',family.code,'name_zh',family.name_zh,'name_en',family.name_en,'description',family.description,'sort_order',family.sort_order,'is_active',family.is_active,'version',family.version) order by family.sort_order,family.name_zh),'[]'::jsonb),count(*)::integer into r,c from public.position_families family where family.property_id=app_private.current_actor_property_id() and (app_private.neon_organization_actor_has_role('property_ld_manager') or exists(select 1 from public.positions position where position.position_family_id=family.id and position.property_id=family.property_id and app_private.neon_position_actor_can_read_position(position.id))); perform app_private.append_neon_position_read_audit('position_families',c); return pg_catalog.jsonb_build_object('rows',r); end $f$;
...
declare r jsonb; c integer; begin perform app_private.assert_neon_organization_hostname(p_hostname); perform app_private.assert_neon_organization_reader(); select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object('id',p.id,'tenant_id',p.tenant_id,'property_id',p.property_id,'position_family_id',p.position_family_id,'code',p.code,'name_zh',p.name_zh,'name_en',p.name_en,'grade_or_band',p.grade_or_band,'is_active',p.is_active,'version',p.version,'department_ids',coalesce((select pg_catalog.jsonb_agg(a.department_id order by a.department_id) from public.position_department_assignments a where a.position_id=p.id and a.is_active and (app_private.neon_organization_actor_has_role('property_ld_manager') or app_private.neon_organization_actor_has_department_scope(a.department_id))),'[]'::jsonb)) order by p.name_zh),'[]'::jsonb),count(*)::integer into r,c from public.positions p where p.property_id=app_private.current_actor_property_id() and app_private.neon_position_actor_can_read_position(p.id); perform app_private.append_neon_position_read_audit('positions',c); return pg_catalog.jsonb_build_object('rows',r); end $f$;
