/* E5A corrective hardening: preserve existing facts, narrow grants, and align contracts. */
begin;

do $e5a_hardening_preflight$
begin
  if current_database() <> 'neondb'
     or current_user <> 'neondb_owner'
     or session_user <> 'neondb_owner' then
    raise exception using errcode = '42501',
      message = 'E5A_EMPLOYEE_WRITE_HARDENING_IDENTITY_MISMATCH';
  end if;
  if not pg_catalog.pg_has_role(
    ...
    raise exception using errcode = '42P01',
      message = 'E5A_EMPLOYEE_WRITE_HARDENING_PREREQUISITE_MISSING';
  end if;
end
$e5a_hardening_preflight$;

/* E5A must not disable or redesign the pre-existing Employee facts boundary. */
do $e5a_restore_fact_trigger$
begin
  if pg_catalog.to_regprocedure(
    ...
    execute 'create trigger employees_record_fact_version '
      'after insert or update on public.employees '
      'for each row execute function app_private.record_employee_fact_version()';
  end if;
end
$e5a_restore_fact_trigger$;

/* Remove only E5A's broad grants, then restore the E2 read union plus E5A writes. */
revoke select, insert, update on public.employees
  from hotel_ld_migration_owner;
revoke select, insert, delete on public.employee_external_identifiers
  from hotel_ld_migration_owner;
...
grant select (
  ...
  employment_status, is_new_employee, is_active, version
...
create or replace function app_private.neon_employee_authoritative_snapshot(
  p_employee_id uuid
)
returns jsonb
language sql
...
create or replace function app_private.append_neon_employee_write_audit(
  p_tenant_id uuid,
  p_property_id uuid,
  p_employee_id uuid,
  p_operation text,
...
begin
  ...
  insert into app_private.employee_write_audit_events (
    request_id, auth_user_id, tenant_id, property_id, employee_id,
    operation, previous_version, result_version,
    before_snapshot, after_snapshot
  ) values (
    app_private.current_actor_request_id(),
    app_private.current_actor_auth_user_id(),
    p_tenant_id, p_property_id, p_employee_id, p_operation,
    p_previous_version, p_result_version,
    p_before_snapshot, p_after_snapshot
  );
end
$function$;

create or replace function public.save_neon_employee_with_identifiers(
  p_hostname text,
  p_tenant_id uuid,
  p_property_id uuid,
  p_employee_id uuid,
...
begin
  ...
  if p_employee_id is null then
    ...
    select employee.version into v_previous_version
    from public.employees employee
    where employee.id = p_employee_id
  ...
  if p_department_id is not null then
    select department.is_active into v_target_active
  ...
  if p_position_id is not null then
    ...
    if v_authoritative_family_id is not null then
      select family.is_active into v_target_active
  ...
  if p_department_id is not null and p_position_id is not null
     and not exists (
       select 1 from public.position_department_assignments assignment
  ...
  select pg_catalog.count(*)::integer into v_identifier_count
  from pg_catalog.jsonb_to_recordset(p_identifiers) identifier(
    source_system text, identifier_type text, identifier_value text,
  ...
  if p_employee_id is null then
    insert into public.employees (
      ...
      p_tenant_id, p_property_id, pg_catalog.btrim(p_employee_number),
      nullif(pg_catalog.btrim(p_name_zh), ''),
      nullif(pg_catalog.btrim(p_name_en), ''),
      p_department_id, p_operational_unit_id, p_position_id,
      v_authoritative_family_id,
      nullif(pg_catalog.btrim(p_grade_or_band), ''),
      p_hire_date, p_probation_or_confirmation_date,
      p_employment_status::public.employee_employment_status,
      p_is_active, 'neon_employee_write', null,
      app_private.current_actor_auth_user_id(),
      app_private.current_actor_auth_user_id()
  ...
  perform app_private.append_neon_employee_write_audit(
    p_tenant_id, p_property_id, v_employee_id,
    case when p_employee_id is null then 'create' else 'update' end,
    v_previous_version, v_result_version,
...
grant execute on function public.save_neon_employee_with_identifiers(
  text,uuid,uuid,uuid,bigint,text,text,text,uuid,uuid,uuid,uuid,
  text,date,date,text,boolean,jsonb
...
begin
  if v_entrypoint is null or not exists (
    ...
    'hotel_ld_migration_owner', 'public.employees', 'is_new_employee', 'UPDATE'
  ) or pg_catalog.has_column_privilege(
    'hotel_ld_migration_owner', 'public.employees', 'source_batch_id', 'SELECT'
  ) or pg_catalog.has_column_privilege(
    'hotel_ld_migration_owner', 'public.employee_external_identifiers',
    'source_batch_id', 'SELECT'
  ) then
    raise exception using errcode = '42501',
      message = 'E5A_EMPLOYEE_WRITE_HARDENING_DRIFT';
  end if;
end
$e5a_hardening_postflight$;
