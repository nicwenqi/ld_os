...
test("workbook parser excludes every sensitive training-evidence category", () => {
  const result = prepareEmployeeMasterStaging(syntheticWorkbookWithTrainingColumns());
  assert.equal(result.safeSummary.excludedColumns.length, 4);
  assert.deepEqual(result.safeSummary.excludedColumns.map(item => item.sourceColumnName), ["CTC", "GTC", "Mini Orientation", "Gender"]);
});
